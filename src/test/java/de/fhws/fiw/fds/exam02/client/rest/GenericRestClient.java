...
public class GenericRestClient<M extends AbstractModel>
{
	...
	private Map<String, Link> parseLinkHeaders(final WebApiResponse<M> webApiResponse)
	{
		...
		final Collection<Link> parsedLinkHeaders = linkHeaders.stream().map(Link::parseFromHttpHeader)
			.collect(Collectors.toList());

		return parsedLinkHeaders.stream().collect(Collectors.toMap(Link::getRelationType, h -> h));
	}

	public RestApiResponse<M> sendGetCollectionRequest(final String url, final GenericType<List<M>> genericType)
		throws IOException
	{
	...
	{
		return new RestApiResponse<>(webApiResponse.getLastStatusCode(), webApiResponse.getResponseData(),
			webApiResponse.getResponseHeaders(), parseLinkHeaders(webApiResponse));
	}
	...
	private RestApiResponse<M> toPostResourceRestApiResponse(final WebApiResponse<M> webApiResponse)
	{
		return new RestApiResponse<>(webApiResponse.getLastStatusCode(),
			webApiResponse.getResponseHeaders().get("Location"), webApiResponse.getResponseHeaders(),
