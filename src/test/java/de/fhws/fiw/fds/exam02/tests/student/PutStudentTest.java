...
public class PutStudentTest extends AbstractStudentTest
{
	...
	@Test public void test_204() throws IOException
	{
	...
	{
		final Student student = new Student("Max", "Mustermann", "max.mustermann@fhws.de", "BIN", 2, 1111);
	...
	@Test public void test_resource_updated_correctly() throws IOException
	{
		...
		final RestApiResponse<Student> responseFromFirstPutRequest = putRequest(HeaderMapUtils.withContentTypeJson(),
			originalStudent);
	...
	{
		final RestApiResponse<Student> responseFromFirstGetRequest = getSingleRequestById(
			HeaderMapUtils.withAcceptJson(), id);
	...
	@Test public void test_conditional_put_204() throws IOException
	{
		...
		final RestApiResponse<Student> responseFromGetRequest = getSingleRequestById(HeaderMapUtils.withAcceptJson(),
			1);
	...
	{
		//user A loads a student
		final RestApiResponse<Student> responseFromGetRequestForUserA = getSingleRequestById(
		...
		final RestApiResponse<Student> responseFromGetRequestForUserB = getSingleRequestById(
			HeaderMapUtils.withAcceptJson(), 1);
		...
		final RestApiResponse<Student> responseFromPutRequestForUserB = putRequest(headersForPutRequestForUserB,
			studentForUserB);
		...
		final RestApiResponse<Student> responseFromPutRequestForUserA = putRequest(headersForPutRequestForUserA,
			studentForUserA);
